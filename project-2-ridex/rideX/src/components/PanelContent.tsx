...
const getVehicleIcon = (vehicleType?: string) => {
  switch (vehicleType?.toLowerCase()) {
    case "motorcycle":
      return <Motorbike size={18} className="w-4 h-4 text-white" />;
    case "bike":
      ...
      {isActive && (
        <div className="mx-5 lg:mx-6 grid grid-cols-2 gap-2">
          <div className="bg-zinc-50 border border-zinc-100 rounded-2xl p-4 flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-zinc-100 flex items-ceter justify-center shrink-0">
          ...
          >
            <div className="rounded-2xl overflow-hidden border border-zinc-100 h-115">
              <RideChat
                ...
                userName={booking?.user?.username || "customer"}
                driverName={booking.driver.username || "driver"}
              />
            </div>
          </motion.div>
        )}
      ...
      {booking?.vehicle && (
        <div className="mx-5 lg:mx-6">
          <div className="bg-zinc-50 border border-zinc-100 rounded-2xl p-4 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[10px] text-zinc-400 uppercase tracking-wider font-semiold">
                Your Vehicle
              </p>
            ...
            <div className="shrink-0 bg-zinc-900 px-3 py-1.5 rounded-lg">
              <p className="text-white text-xs font-black tracking-widest font-momo">
                {booking.vehicle.licensePlate ?? "number"}
              </p>
            </div>
          </div>
        </div>
