  ...
  return (
    <div className="space-y-3">
      ...
      {data.map((item: any, index: number) => {
        const name =
          item.name || item.owner?.username || item.username || "User";
        const email = item.email || item.owner?.email || "No email";

        // ← key fix: only allow KYC actions after docs approved (step 4)
        const docsApproved = item.partnerOnBoardingSteps === 4;
        ...
        return (
          ...
          >
            ...
            <div className="shrink-0">
              {docsApproved && item.videoKycStatus === "pending" ? (
                <motion.button
                  whileTap={{ scale: 0.96 }}
              ...
              ) : docsApproved && item.videoKycStatus === "in_progress" ? (
                <motion.button
                  whileTap={{ scale: 0.96 }}
                  className="flex items-center gap-2 px-4 py-2 rounded-xl bg-neutral-950 hover:bg-neutral-800 text-white text-sm font-semibold transition-colors"
                  onClick={() =>
