...
function Page() {
  ...
  const handleAccept = async (id: string) => {
    try {
      const { data } = await axios.get(
        `/api/partner/booking/${id}/accept-booking`
      );
  ...
  const handleReject = async (id: string) => {
    try {
      const { data } = await axios.get(
        `/api/partner/booking/${id}/reject-booking`
      );
      window.location.reload();
    } catch (error) {
      console.log(error);
                  ...
                  <div className="flex flex-col justify-between lg:items-end gap-6 w-full lg:w-auto">
                    ...
                    <div className="flex gap-4 w-full lg:w-auto">
                      <button
                        ...
                        className="flex-1 lg:flex-none px-8 py-3 rounded-xl  bg-black text-white text-sm font-semibold hover:bg-gray-900 hover:shadow-lg transition-all duration-200 active:scale-[0.98] disabled:opacity-50 flex items-center justify-center
                    "
                      >
                        Accept Ride
                      </button>
