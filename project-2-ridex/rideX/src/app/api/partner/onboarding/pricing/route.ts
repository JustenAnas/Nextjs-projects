...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!vehicle) {
      return Response.json({ message: "cant find  vehicle" }, { status: 400 });
    }
    ...
    if (image && image.size > 0) {
      const imageUrl = await uploadOnCloudniary(image);
...
export async function GET() {
  try {
    ...
    return Response.json({ message: `pricing get error` }, { status: 500 });
  }
}
